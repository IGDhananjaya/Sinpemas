...
class OrmasController extends Controller
{
    ...
    //     $validated = $request->validate([
    //         'nama_ormas' => 'required|string|max:255',
    //         'bentuk_organisasi_id' => 'required|exists:bentuk_organisasis,id', // Validasi bentuk organisasi
    //         'bidang_id' => 'nullable|array',
    //         'bidang_id.*' => 'exists:bidangs,id', // Validasi untuk bidang
    //         'surat_permohonan' => 'nullable|file|mimes:pdf|max:2048',
    //         'surat_penetapan' => 'nullable|file|mimes:pdf|max:2048',
    //         'sk_susunan_kepengurusan' => 'nullable|file|mimes:pdf|max:2048',
    //         'plang_nama_sekretariat' => 'nullable|image|mimes:jpeg,png,jpg,gif|max:2048',
    ...
    //             $originalName = $request->file($field)->getClientOriginalName();
    //             $validated[$field] = $request->file($field)->storeAs('ormas_files', $originalName);
    //         } else {
    //             // Jika tidak ada file baru, gunakan file yang sudah ada
    //             $validated[$field] = $orma->$field; // Menggunakan file yang sudah ada
    //         }
    //     }

    //     // Pertahankan user_id yang ada
    //     $validated['user_id'] = $orma->user_id; // Memastikan user_id tetap sama

    //     // Update bidang_id jika ada
    //     if ($request->has('bidang_id')) {
    //         $orma->bidang()->sync($validated['bidang_id']); // Sinkronisasi bidang
    //     }

    //     // Update data di database, termasuk bentuk organisasi
    //     $validated['bentuk_organisasi_id'] = $request->bentuk_organisasi_id; // Simpan bentuk organisasi
